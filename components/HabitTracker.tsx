...
export default function HabitTracker() {
  ...
  const activeDays = useMemo(() => dailyDurationMap(sessions), [sessions]);
  const curStreak = useMemo(() => currentStreak(sessions), [sessions]);
  const bstStreak = useMemo(() => bestStreak(sessions), [sessions]);
  ...
  const cells = useMemo<CalCell[]>(() => {
    ...
    for (let i = 0; i < firstDow; i++) {
      result.push({ day: null, dateStr: null, isToday: false, isFuture: false, hasSession: false });
    }
    ...
    return result;
  }, [viewYear, viewMonth, daysInMonth, firstDow, activeDays, todayStr]);

  // Stats for the viewed month
  const monthSessionDays = useMemo(
    () => Array.from(activeDays.keys()).filter((d) =>
  ...
  const daysWithGe1h = Array.from(activeDays.values()).filter((s) => s >= 3600).length;
  const daysWithGe2h = Array.from(activeDays.values()).filter((s) => s >= 7200).length;
  ...
  const totalSecs = Array.from(activeDays.values()).reduce((a, b) => a + b, 0);
  const avgDailyFocus = activeDays.size > 0 ? totalSecs / activeDays.size : 0;
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-7 mb-1">
            {DOW_LABELS.map((label) => (
              <div key={label} className="flex items-center justify-center text-[10px] font-medium text-[hsl(var(--muted))] h-6">
                {label}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-y-1">
            {cells.map((cell, idx) => {
              if (cell.day === null) return <div key={idx} className="flex items-center justify-center h-8" />;
                ...
                <div key={idx} className="flex items-center justify-center">
                  <div
                    ...
                    className={[
                      ...
                      filled ? "bg-[hsl(var(--primary))]" :
                      cell.isToday ? "border-2 border-[hsl(var(--primary))] bg-transparent" :
                      faint ? "opacity-25" : "border border-[hsl(var(--hairline))] bg-transparent",
                    ].join(" ")}
                  >
                    {filled ? (
                      <span className="text-[11px] font-semibold text-white leading-none select-none">✓</span>
                    ) : (
                      <span className={["text-[11px] leading-none select-none", faint ? "text-[hsl(var(--muted))]" : cell.isToday ? "text-[hsl(var(--primary))] font-semibold" : "text-[hsl(var(--muted))]"].join(" ")}>
                        {cell.day}
                      </span>
                    )}
                  </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { icon: "🔥", value: `${curStreak} day${curStreak !== 1 ? "s" : ""}`, label: "streak" },
            { icon: "📅", value: `${monthSessionDays}/${daysInMonth}`, label: "this month" },
            { icon: "🏆", value: `${bstStreak} day${bstStreak !== 1 ? "s" : ""}`, label: "best" },
          ].map((stat) => (
            <div key={stat.label} className="flex flex-col items-center gap-0.5 rounded-lg p-2 bg-[hsl(var(--surface-strong))]">
              <span className="text-base leading-none">{stat.icon}</span>
              <span className="text-[13px] font-semibold text-[hsl(var(--body-strong))] leading-tight">{stat.value}</span>
              <span className="text-[10px] text-[hsl(var(--muted))] leading-none">{stat.label}</span>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          {showStats && (
            <div className="mt-2 rounded-lg bg-[hsl(var(--surface-strong))] p-3 space-y-1.5">
              <p className="text-[12px] font-semibold text-[hsl(var(--body-strong))] mb-2">📊 Habit Statistics</p>
              {[
                ...
                { label: "This week (Mon–Sun)", value: `${thisWeekDays}/7 days` },
                { label: "This month", value: `${thisMonthDays}/${daysInCurrentMonth} days` },
                ...
                { label: "Average daily focus", value: avgDailyFocus > 0 ? formatHM(avgDailyFocus) : "—" },
              ].map((row) => (
                <div key={row.label} className="flex items-baseline justify-between gap-2">
                  <span className="text-[11px] text-[hsl(var(--muted))] shrink-0">• {row.label}</span>
                  <span className="text-[11px] font-medium text-[hsl(var(--body-strong))] text-right tabular-nums">{row.value}</span>
                </div>
              ))}
            </div>
          )}
