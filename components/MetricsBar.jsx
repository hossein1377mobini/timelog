...
export default function MetricsBar() {
  ...
  function load() {
    setSessions(safeParseSessions(localStorage.getItem("compass_sessions")))
  }

  const now = new Date()
  const todaySessions = sessions.filter((s) => new Date(s.startedAt).toDateString() === now.toDateString())
  ...
  const prevWeekSeconds = prevWeekSessions.reduce((sum, x) => sum + x.duration, 0)
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {metrics.map((m) => (
        <div key={m.label} className="bg-[hsl(var(--surface-card))] rounded-[12px] border border-[hsl(var(--hairline))] px-4 py-3">
          <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[hsl(var(--muted))] mb-1">
            {m.label}
          </p>
          <p className={`text-[22px] font-normal leading-[1.3] tracking-[-0.11px] mb-1 ${m.green ? "text-[hsl(var(--success))]" : "text-[hsl(var(--body-strong))]"}`}>
            {m.value}
          </p>
